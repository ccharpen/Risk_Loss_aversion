...
function Pgen = generate_choice_split_per_session(x,P)
...
for i=1:length(P)
    ...
    elseif P(i,10)==2 %anticipation session
        Ugamble(i) = 0.5*sign(P(i,4))*(abs(P(i,4)))^x(6) + 0.5*x(5)*sign(P(i,3))*(abs(P(i,3)))^x(6);
        Usure(i) = sign(P(i,2))*(abs(P(i,2)))^x(6);
        xb = x(4)*(Ugamble(i) - Usure(i));
    elseif P(i,10)==3 %recovery session
        Ugamble(i) = 0.5*sign(P(i,4))*(abs(P(i,4)))^x(9) + 0.5*x(8)*sign(P(i,3))*(abs(P(i,3)))^x(9);
        Usure(i) = sign(P(i,2))*(abs(P(i,2)))^x(9);
        xb = x(7)*(Ugamble(i) - Usure(i));
    end
